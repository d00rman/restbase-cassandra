...
DB.prototype._put = function(req) {
    ...
    // log an error with the status code of the main update so we can
    // find out if in travis we can see it
    update = update.then(function(result) {
        self.log('error/cassandra/backgroundUpdates', {msg: 'Status before updates', status: result.status});
    });
